...
def enrich_with_omniparser(elements: list[dict], png_path: Path) -> list[dict]:
    """Upgrade label on icon-only Images using OmniParser captions.

    For any element whose label is generic ("AXImage" / "Image" / blank), we
    look up an OmniParser detection that overlaps significantly and use its
    caption. Silently skips if the model isn't installed.
    """
    try:
        from run_omniparser import have_models, detect_icons, caption_icons  # type: ignore
    except Exception:
        return elements
    if not have_models():
        return elements
    ...
    try:
        items = detect_icons(png_path)
        items = caption_icons(png_path, items)
    except Exception as e:
        print(f"  ⚠ OmniParser enrichment failed: {e}")
        return elements

    import requests as _rq
    try:
        mons = _rq.get(f"{API}/screen/monitors", timeout=3).json()
        scale = float(mons[0]["scale_factor"] or 2.0)
    except Exception:
        scale = 2.0

    # Convert OmniParser bboxes (physical px) to logical
    omni = []
    for it in items:
        x, y, w, h = it["bbox_px"]
        omni.append({
    ...
    for e in elements:
        is_generic = (
            e.get("role", "").endswith("Image")
            and e["label"] in ("AXImage", "Image", "")
        )
        if not is_generic:
            continue
        best = None
        ...
        for o in omni:
            ...
            if iou > best_iou and iou > 0.4:
                ...
                best = o
        if best:
            e["label"] = best["label"][:60]
            e["source"] = "ax+omni"
            upgraded += 1
    if upgraded:
        print(f"  OmniParser enriched {upgraded} icon labels")
    return elements
